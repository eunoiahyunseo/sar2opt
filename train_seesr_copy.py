...
for epoch in range(first_epoch, args.num_train_epochs):
    for step, batch in enumerate(train_dataloader):
        ...
        with accelerator.accumulate(controlnet), accelerator.accumulate(unet), accelerator.accumulate(vit_to_sd_proj):
            ...
            pred_original_latents = (noisy_latents - sqrt_one_minus_alpha_prod * model_pred) / sqrt_alpha_prod
            ...
            if accelerator.is_main_process:
                ...
                if global_step % args.checkpointing_steps == 0:
                    with torch.no_grad():
                        ...
                        del pipeline, model, validation_dataloader, validation_dataset
                        torch.cuda.empty_cache()

        logs = {"loss": loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0], "perceptual_loss": perceptual_loss.detach().item(), "mse_loss": mse_loss.detach().item()}
