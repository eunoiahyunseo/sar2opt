...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, idx_to_group_img=None, image_atts=None, condition=None, return_hierarchical_features=False, **kwargs):
        x = self.patch_embed(x)
        if self.ape:
            x = x + self.absolute_pos_embed
        x = self.pos_drop(x)
        hierarchical_features = [] # 1. 계층적 특징을 담을 리스트


        for i, layer in enumerate(self.layers):
            x = layer(x, condition=condition)
            if return_hierarchical_features:
                # 각 Stage의 출력을 풀링하여 리스트에 추가
                # avgpool은 (B, C, L) 입력을 받으므로 transpose 필요
                pooled_feature = self.avgpool(x.transpose(1, 2))
                hierarchical_features.append(torch.flatten(pooled_feature, 1))


        x = self.norm(x)  # B L C

        # x_cls = self.avgpool(x.transpose(1, 2))  # B C 1
        final_feature_pooled = self.avgpool(x.transpose(1, 2))  # B C 1

        original_output = torch.cat([final_feature_pooled.transpose(1, 2), x], dim=1) # Shape: [B, L+1, C]

        if return_hierarchical_features:
            return original_output, hierarchical_features

        if idx_to_group_img is None:
            # This path is what `condition_forward` in RAM seems to use conceptually.
            # It returns a "CLS" token and the sequence of patch tokens.
            # For simplicity in DAPE, we only need the features that get projected.
            # x_cls = final_feature_pooled
