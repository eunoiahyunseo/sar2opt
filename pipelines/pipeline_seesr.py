...
@dataclass
class StableDiffusionControlNetPipelineOutputWithConfidence(StableDiffusionPipelineOutput):
    """
    Output class for Stable Diffusion ControlNet pipeline with confidence maps.

    Args:
        images (`List[PIL.Image.Image]` or `np.ndarray`):
            List of denoised PIL images of length `batch_size` or numpy array of shape `(batch_size, height, width,
            num_channels)`. PIL images or numpy array present the denoised images of the diffusion process.
        nsfw_content_detected (`List[bool]`):
            List of flags denoting whether the corresponding generated image likely represents "not-safe-for-work"
            (nsfw) content, or `None` if safety checking could not be performed.
        confidence_maps (`List[PIL.Image.Image]` or `np.ndarray`):
            List of confidence maps corresponding to the generated images.
    """
    confidence_maps: Union[List[PIL.Image.Image], np.ndarray]
...
class StableDiffusionControlNetPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                if h*w<=tile_size*tile_size: # tiled latent input
                    ...
                    noise_pred, conf_pred = torch.split(full_output, [self.unet.config.out_channels, 1], dim=1)
                ...
                if i == len(timesteps) - 1 and output_confidence_map:
                    # Classifier-free guidance가 적용된 경우, 조건부 예측에 해당하는 부분만 사용
                    if do_classifier_free_guidance:
                        # conf_pred의 shape: (2 * batch_size, 1, H, W)
                        _, conf_pred_text = conf_pred.chunk(2)
                        last_step_confidence_map = conf_pred_text
                    else:
                        last_step_confidence_map = conf_pred
        ...
        if output_confidence_map and last_step_confidence_map is not None:
            # 신뢰도 맵 후처리 (softplus -> 정규화 -> 이미지 변환)
            confidence_map = F.softplus(last_step_confidence_map)
            
            # 각 이미지별로 0-1 정규화
            for j in range(confidence_map.shape[0]):
                map_slice = confidence_map[j]
                map_slice = (map_slice - map_slice.min()) / (map_slice.max() - map_slice.min() + 1e-6)
                confidence_map[j] = map_slice
            
            # VAE의 latent와 동일한 크기이므로, 이미지 크기로 업샘플링
            confidence_map = torch.nn.functional.interpolate(
                confidence_map,
                size=(height, width),
                mode="bicubic",
                align_corners=False,
            ...
            # 흑백 이미지를 컬러로 변환 (채널 복제)
            confidence_map = confidence_map.repeat(1, 3, 1, 1).clamp(0, 1)
            
            # VaeImageProcessor를 사용하여 PIL 이미지나 numpy 배열로 변환
            confidence_map_image = self.image_processor.postprocess(confidence_map, output_type=output_type)
        ...
        return StableDiffusionControlNetPipelineOutputWithConfidence(
            images=image, 
            nsfw_content_detected=has_nsfw_concept, 
            confidence_maps=confidence_map_image
        )